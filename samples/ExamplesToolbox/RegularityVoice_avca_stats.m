function mFeatures= RegularityVoice_avca_stats (sFile)
% Calculates the regularity features statistics of the input audio file
% (sFile)
...
mFeatures= [ mean(rApEn), std(rApEn),...
    mean(rSampEn), std(rSampEn), mean(rFuzzyEn), std(rFuzzyEn), ...
    mean(rGSampEn), std(rGSampEn), mean(rmSampEn), std(rmSampEn)];
